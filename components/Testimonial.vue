<template>
  <div class="text-bg-dark" style="height: 600px">
    <div class="container">
      <input type="radio" name="nav" id="first" checked />
      <input type="radio" name="nav" id="second" />
      <input type="radio" name="nav" id="third" />

      <label for="first" class="first"></label>
      <label for="second" class="second"></label>
      <label for="third" class="third"></label>

      <div class="one slide">
        <blockquote>
          <span class="leftq quotes">&ldquo;</span>Je devais ameliorer ma
          communication en English très rapidement. Je devais le faire pour
          pouvoir presenter Itot Africa à des investisseurs qui ne parle pas
          français. Alors j'avais fait appel à Patrick. Il à commencer à coacher
          toute l'equipe Itot Africa. Grâce à son coaching, j'avais bien
          présenté Itot Africa, expliqué notre model financier, etc. On a obtenu
          le prêt qu'on voulait. Nous savons que vous aussi avez besoins de son
          coaching. <span class="rightq quotes">&bdquo; </span>
        </blockquote>
        <img
          src="data:image/jpeg;base64,"
          width="170"
          height="130"
        />
        <h2>Samy Mwamba</h2>
        <h6>CEO - ITOT AFRICA</h6>
      </div>

      <div class="two slide">
        <blockquote>
          <span class="leftq quotes">&ldquo;</span> C'était et ce sera toujours
          un plaisir pour moi de travailler avec vous. Merci pour la formation
          en anglais, car elle m'a permis de progresser et de franchir une
          nouvelle étape dans ma vie. Je vous souhaite d'être reconnu comme le
          meilleur coach d'anglais de notre pays. Continuez à faire du bon
          travail, continuez à évaluer le travail, continuez à aller de l'avant.
          Dites à vos apprenants cette citation de Denzel Washington : "Pour
          obtenir quelque chose que vous n'avez jamais eu, vous devez faire
          quelque chose que vous n'avez jamais fait.<span class="rightq quotes">&bdquo; </span>
        </blockquote>
        <img
          src="../assets/images/testimonial/Herve.jpg"
          width="170"
          height="130"
        />
        <h2>Hervé Kapuya</h2>
        <h6>▪︎ Administrateur - Electronic Congo Development (e-CD)</h6>
      </div>

      <div class="three slide">
        <blockquote>
          <span class="quotes leftq"> &ldquo;</span> He promptly completed the
          task at hand and communicates really well till the project reaches the
          finishing line. I was pleased with his creative design and will
          definitely be hiring him again.
          <span class="rightq quotes">&bdquo; </span>
        </blockquote>
        <img
          src="http://media.digitalcameraworld.com/files/2012/10/People_photography_composition_tips_PHG18.chap4_.n_p38_39.jpg"
          width="170"
          height="130"
        />
        <h2>Steve Stevenson</h2>
        <h6>CEO Woof Web Design Studios</h6>
      </div>
    </div>
  </div>
</template>

<script>
export default {};
</script>

<style lang="scss" scoped>
// @import url(https://fonts.googleapis.com/css?family=Open+Sans);

a {
  color: #ccc;
}
.container {
  width: 500px;
//   min-height: 100%;
height: 490px;
  margin: 0 auto;
  position: relative;
  padding-bottom: 30px;
  overflow: hidden;
}
h1 {
  text-align: center;
  text-shadow: 0 1px white;
  color: #02303f;
}
h2 {
  color: #736861;
  margin: 15px 0 5px;
  text-shadow: 0 1px rgba(255, 255, 255, 0.5);
}
h6 {
  color: #928566;
  margin: 0;
}
input[type="radio"] {
  position: absolute;
  width: 1px; /* Setting this to 0 make it invisible for VoiceOver */
  height: 1px; /* Setting this to 0 make it invisible for VoiceOver */
  padding: 0;
  margin: -1px;
  border: 0;
  clip: rect(0 0 0 0);
  overflow: hidden;
}
label {
  display: block;
  width: 32%;
  border: 4px solid white;
  position: absolute;
  bottom: 5px;
  cursor: pointer;
  transition: border-color 0.3s linear;
}

label.second {
  left: 34%;
}
label.third {
  left: 68%;
}

blockquote {
  margin: 0;
  padding: 30px;
  width: 500px;
  background-color: $title_color;
  color: white;
  box-shadow: 0 5px 2px rgba(0, 0, 0, 0.1);
  position: relative;
  transition: background-color 0.6s linear;
}

blockquote:after {
  content: " ";
  height: 0;
  width: 0;
  position: absolute;
  top: 100%;
  border: solid transparent;
  border-top-color: $title_color;
  border-left-color: $title_color;
  border-width: 10px;
  left: 10%;
}
#second:checked ~ .two blockquote {
  background-color: #989898;
}
.two blockquote:after {
  border: solid transparent;
  border-top-color: #989898;
  border-left-color: #989898;
  border-width: 10px;
}
#third:checked ~ .three blockquote {
  background-color: #54885f;
}
.three blockquote:after {
  border: solid transparent;
  border-top-color: #54885f;
  border-left-color: #54885f;
  border-width: 10px;
}
.quotes {
  position: absolute;
  color: rgba(255, 255, 255, 0.5);
  font-size: 5em;
}
.leftq {
  top: -25px;
  left: 5px;
}
.rightq {
  bottom: -10px;
  right: 5px;
}
img {
  float: left;
  margin-right: 20px;
}
.slide {
  position: absolute;
  left: -100%;
  opacity: 0;
  transition: all 0.6s ease-in;
}

#first:checked ~ label.first {
  border-width: 6px;
  border-color: $title_color;
}
#second:checked ~ label.second {
  border-width: 6px;
  border-color: #989898;
}
#third:checked ~ label.third {
  border: 6px solid #54885f;
}

#first:checked ~ div.one {
  left: 0;
  opacity: 1;
}
#second:checked ~ div.two {
  left: 0;
  opacity: 1;
}
#third:checked ~ div.three {
  left: 0;
  opacity: 1;
}
</style>